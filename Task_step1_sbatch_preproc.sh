...
#SBATCH -J "TaskS1"   # job name
####SBATCH --mail-type=ALL #email notifications
####SBATCH --mail-user=kirwan@byu.edu # <--update your email address
...
phaseArr=(knowfilt)                    # Each PHASE of experiment, in order (TEST1 precedes TEST2 but followed STUDY). This is absolutely necessary - so put something here
...
if [ ! -s anat.un.aff.Xat.1D ]; then
    ...
    #3dbucket -prefix struct_warped awpy/struct_ns.aw.nii*                  ## The old command was `3dbucket -prefix struct_ns awpy/struct_ns.aw.nii*`, but that conflicted with an existing dataset, so AFNI balked and didn't do it. Consequently, I changed the filename to struct_warped+tlrc
    #3drefit -view tlrc -space MNI struct_warped+orig
    #3dcopy awpy/struct_ns.aw.nii struct_warped+tlrc #should copy the header info.
