...
#SBATCH --mail-user=kirwan@byu.edu
...
# General variables
workDir=~/compute/KnowFiltMISQ/derivatives						# par dir of data
outDir=${workDir}/grp-Conf-Disconf          						# where output will be written (should match step3)
refFile=${workDir}/sub-8107/run-01_knowfilt_scale+tlrc			# reference file, for finding dimensions etc

tempDir=~/compute/KnowFiltMISQ/code/templates    				# desired template
...
compList=(knowfilt)	            								# matches decon prefixes, and will be prefix of output files
compLen=${#compList[@]}


## confirm vs. disconfirm
arrA=(3)    												# setA beh sub-brik for compList. Must be same length as compList
arrB=(5)    												# setB
listX=AB													# list of arr? used, for building permutations (e.g. listX=ABC)

namA=(confirm)										# names of behaviors from arrA. Must be same length as arrA
namB=(disconfirm)
...
bsArr=()												# Bx-subject variables (groups)
